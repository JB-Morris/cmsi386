 ...
 * 1 sieve, since there are fewer than 100 primes less thna 100.
 *
...
class DivideFilter {
	...
	this.data = new int[this.maximumCapacity];
    }
	...
	// throw new RuntimeException("Implement me");
    	if(i < 1){
    		throw new NoMoreRoomException(); //tests would not allow for an IllegalArgumentException
    	}
    	if(this.currentAmount == maximumCapacity){
    		throw new NoMoreRoomException();
    	}
    	this.data[currentAmount] = i;
    	this.currentAmount++;
        return;
...
class TestDivideFilter {
    ...
    void test2() {
	DivideFilter filter = new DivideFilter(1);
	assert(filter.anyEvenlyDivides(137) == false);
...
class Printer implements Runnable {
	...
	// then return.
    	int index = Helpers.take(this.input);
        while(index != -1){
            System.out.println(index);
            index = Helpers.take(this.input);
        }
        System.out.println(index);
        return;
...
class TestPrinter {
	public static void main(String[] args) {
		// BlockingQueue<Integer> testQueue = new ArrayBlockingQueue<Integer>();
		// for(int i = 0; i < 10; i++){
		// 	Helpers.put(testQueue, i);	
		// }
		// Helpers.put(testQueue, -1);

		// Printer testPrinter = new Printer(testQueue);
		// Thread t = new Thread(testPrinter);
		// t.start();
...
class Sieve implements Runnable {
    ...
    public void run() {
        ...
        while(!done){
            ...
            int temp = Helpers.take(this.input);
            ...
            if(!(this.filter.anyEvenlyDivides(temp))){
                // Helpers.put(this.output, temp);
                // Helpers.take(this.input);
                ...
                if(this.filter.full()){
                    if(!interrupted){
                        BlockingQueue<Integer> first = new ArrayBlockingQueue<Integer>(this.queueSize);
                        BlockingQueue<Integer> newer = output;
                        Sieve newSieve = new Sieve(first, newer, this.filterSize, this.queueSize);
                        this.output = first;
                        Thread example = new Thread(newSieve);
                        example.start();
                        interrupted = true;
                    } else {
                        Helpers.put(this.output, temp);
...
class HW6 {
    ...
    Generator generator = new Generator(max, input);
    Thread startGen = new Thread(generator);
    startGen.start();
    Sieve sieve = new Sieve(input, output, filterSize, queueSize);
    Thread startSieve = new Thread(sieve);
    startSieve.start();
    Printer printer = new Printer(output);
    Thread startPrinter = new Thread(printer);
    startPrinter.start();
    try{
        Helpers.join(startSieve);
        Helpers.join(startPrinter);
    }catch(Exception e){
 ...
 *
 * 1) How many CPUs cores does your machine have? 4
 * 
 * 2) What is the run time for each of:
 *      java HW6 10000 1 1 - 1.62
 *      java HW6 10000 1 10 - .36
 *      java HW6 10000 10 1 - .18
 *      java HW6 10000 10 10 - .19
 *    What conclusions can you make from these times?
 *    Running multiple threads even with a smaller queue size makes for a faster run time.
 * 
 ...
 *      java HW6 100000 10 10 - 100%
 *      java HW6 100000 10000 10 - 27%
 *    What conclusions can you make from these observations?
 *    More threads make for more CPU usage.
 *
 ...
 *    10000000 10000 100 - 3:16.55 (laptop) 2:39.61 (desktop)
