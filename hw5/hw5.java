...
class Num implements AExp {
    ...
    public List<AInstr> compile(){
      List<AInstr> copyList = new LinkedList<AInstr>();
      copyList.add(new Push(val));
      return copyList;
    }
}
...
class BinOp implements AExp {
    ...
    public BinOp(AExp first, Op ea, AExp second){
      left = first;
      op = ea;
      right = second;
    }
    ...
    public Double eval(){
      Double value = op.calculate(left.eval(), right.eval());
      return value;
    }

    public List<AInstr> compile(){
      List<AInstr> copyList = new LinkedList<AInstr>();
      copyList.addAll(left.compile());
      copyList.addAll(right.compile());
      copyList.add(new Calculate(op));
      return copyList;
    }
}
...
class Calculate implements AInstr {
    protected Op op;
    protected Double ex1,ex2,result;

    public Calculate(Op op1){
      op = op1;
    }
...
class Instrs {
    ...
    protected AInstr instruction;
    protected Stack<Double> doubles = new Stack<Double>();
    ...
    public Double eval() {
      for(AInstr i : this.instrs){
        instruction = i;
        instruction.eval(doubles);
      }
      output = doubles.pop();
      return output;
    }  // Problem 2
